...
results_mean <- results_count %>% 
  f_ribbon_dataprep_mean_lowvolume(variable = sex)

# Join count and mean data
results_toplot <- results_count %>%
  dplyr::left_join(results_mean, by = c("sex", "event_month")) %>%
  #
  dplyr::filter(event_yearmonth >= ribbon_start)

# Create and format ribbon chart
f_ribbon_format_historical_yes(results_toplot,
                               chart_title = charttitle_list[i]) +
  #
  facet_wrap(~sex,
             axes = "all")
...
# Mean and upper/lower limits for each month
results_mean <- results_count %>% 
  f_ribbon_dataprep_mean_lowvolume(variable = age_group)

# Join count and mean data
results_toplot <- results_count %>%
  dplyr::left_join(results_mean, by = c("age_group" ,"event_month")) %>%
  #
  dplyr::filter(event_yearmonth >= ribbon_start)
...
# Mean and upper/lower limits for each month
results_mean <- results_count %>% 
  f_ribbon_dataprep_mean_lowvolume(variable = local_government_area)

# Join count and mean data
results_toplot <- results_count %>%
  dplyr::left_join(results_mean, by = c("local_government_area", "event_month")) %>%
  #
  dplyr::filter(event_yearmonth >= ribbon_start)
