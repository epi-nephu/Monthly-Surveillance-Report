...
version_details: "v2.0, 31/10/2025"
output: 
  html_document:
    code_folding: none
    toc: true
...
# Change log
# v1.0: First version
# v2.0: Updated to reflect sourcing data directly from PHAR
```
...
## Part 2: A Quick Overview of Data Sources
...
In addition to the PHESS data, there are also three reference documents/files required. These don't need to be updated each month.
...
There have been some recent (August/September 2025) changes to how Mailchimp handles images (in summary, it no longer renders them directly from a .txt file), so we now need to send Comms the External Report html along with the .png files for each chart we want to include in the report. Comms will build the report in Mailchimp.

The .pngs can be found in the figure-html subfolder that is created when you knit the report. Each chart is outputted as a separate .png into this folder.

```{r, out.width = '600px'}
#
knitr::include_graphics(here::here("Screenshots", "comms_charts.PNG"))
```

<br/>

The html and png files are sent as attachments via email to the NEPHU Comms Team.
...
<!-- # Appendix 1: Conditional Logic Flowcharts -->
