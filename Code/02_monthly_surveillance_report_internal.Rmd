---
title: 'NEPHU Monthly Surveillance Report - Internal - February 2026'
output: 
  html_document: 
    code_folding: none
    toc: true
    ...
    self_contained: true
knit: (function(input, ...) {
  rmarkdown::render(input, output_file = "Monthly Surveillance Report 2026 02 Internal", output_dir = "../Reports") })
---
...
for(i in seq_along(conditions_list)) {
  ...
  if (total_n_four_year[i] > 0 & historical_data[i] == "Yes" & mean_four_year[i] >= high_volume) {
    
    results[[i]] <- knitr::knit_child(here::here("Code", "Child Rmds", "02d_analyses_detailed_highvolume.Rmd"), 
                                      quiet = TRUE, envir = environment())

  } else if (total_n_four_year[i] > 0 & historical_data[i] == "Yes" & mean_four_year[i] < high_volume) {

    results[[i]] <- knitr::knit_child(here::here("Code", "Child Rmds", "02d_analyses_detailed_lowvolume.Rmd"), 
                                      quiet = TRUE, envir = environment())

  } else if (total_n_four_year[i] > 0 & historical_data[i] == "No") {
...
* Additional notes on interpreting the summary tables:
    ...
    * The comparison column indicates whether the monthly rate in NEPHU was higher higher (↑) or lower (↓) than for Victoria overall.
